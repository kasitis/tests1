...
// CHANGED: Added imports for all required icons
import { ArrowLeft, Trash2, Download, Upload, FileSpreadsheet, Image, Edit2 } from 'lucide-react';

import { MAX_XLSX_IMPORT_ERRORS_TO_SHOW, APP_FIELDS_FOR_MAPPING, MAX_FORM_OPTIONS } from '../constants.ts';
...
const QuestionBankView: React.FC = () => {
  ...
  const filteredQuestions = React.useMemo(() => {
    ...
    }).sort((a,b) => a.question.localeCompare(b.question)); // Sort alphabetically
  ...
  const processAndImportMappedData = useCallback((
    ...
    if (!questionTextMapped && APP_FIELDS_FOR_MAPPING.find(f => f.key === 'question')?.required) {
  ...
  const handleXlsxCsvFileSelect = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
        try {
            ...
            const sheetDataArray: string[][] = window.XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: "" });
